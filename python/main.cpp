...
PYBIND11_MODULE(_QPot, m)
{
    ...
    py::class_<QPot::Chunked>(m, "Chunked")
        ...
        .def(
            py::init<double, xt::xtensor<double, 1>, long>(),
            "Chunked yield sequence."
            "See :cpp:class:`QPot::Chunked`.",
            py::arg("x"),
            py::arg("y"),
        ...
        .def(
            "set_y",
            py::overload_cast<long, const xt::xtensor<double, 1>&>(
                &QPot::Chunked::set_y<xt::xtensor<double, 1>>),
            "Reset the chunk."
        ...
        .def(
            "set_y",
            py::overload_cast<const xt::xtensor<double, 1>&, long>(
                &QPot::Chunked::set_y<xt::xtensor<double, 1>>),
            "Reset the chunk."
        ...
        .def(
            "rshift_y",
            py::overload_cast<long, const xt::xtensor<double, 1>&, size_t>(
                &QPot::Chunked::rshift_y<xt::xtensor<double, 1>>),
            "Right-shift chunk."
        ...
        .def(
            "rshift_y",
            py::overload_cast<const xt::xtensor<double, 1>&, size_t>(
                &QPot::Chunked::rshift_y<xt::xtensor<double, 1>>),
            "Right-shift chunk."
        ...
        .def(
            "rshift_dy",
            py::overload_cast<long, const xt::xtensor<double, 1>&, size_t>(
                &QPot::Chunked::rshift_dy<xt::xtensor<double, 1>>),
            "Right-shift chunk."
        ...
        .def(
            "lshift_y",
            py::overload_cast<long, const xt::xtensor<double, 1>&, size_t>(
                &QPot::Chunked::lshift_y<xt::xtensor<double, 1>>),
            "Left-shift chunk."
        ...
        .def(
            "lshift_dy",
            py::overload_cast<long, const xt::xtensor<double, 1>&, size_t>(
                &QPot::Chunked::lshift_dy<xt::xtensor<double, 1>>),
            "Left-shift chunk."
        ...
        .def(
            "shift_y",
            py::overload_cast<long, const xt::xtensor<double, 1>&, size_t>(
                &QPot::Chunked::shift_y<xt::xtensor<double, 1>>),
            "Shift chunk."
        ...
        .def(
            "shift_dy",
            py::overload_cast<long, const xt::xtensor<double, 1>&, size_t>(
                &QPot::Chunked::shift_dy<xt::xtensor<double, 1>>),
            "Shift chunk."
        ...
        .def(
            "redraw",
            py::overload_cast<>(&QPot::Chunked::redraw, py::const_),
            "Check for redraw."
            "See :cpp:func:`QPot::Chunked::redraw`.")

        .def(
            "redraw",
            py::overload_cast<double>(&QPot::Chunked::redraw, py::const_),
        ...
        .def(
            "yleft",
            py::overload_cast<>(&QPot::Chunked::yleft, py::const_),
            "Yield position directly left of x()."
            "See :cpp:func:`QPot::Chunked::yleft`.")

        .def(
            "yright",
            py::overload_cast<>(&QPot::Chunked::yright, py::const_),
            "Yield position directly right of x()."
            "See :cpp:func:`QPot::Chunked::yright`.")

        .def(
            "yleft",
            py::overload_cast<size_t>(&QPot::Chunked::yleft, py::const_),
        ...
        .def(
            "yright",
            py::overload_cast<size_t>(&QPot::Chunked::yright, py::const_),
    ...
    py::class_<QPot::Static>(m, "Static")
        ...
        .def(
            "setProximity",
            &QPot::Static::setProximity,
            "Customise proximity search",
            py::arg("proximity"))

        .def("setPosition", &QPot::Static::setPosition, "Update current position", py::arg("x"))
        ...
        .def(
            "currentIndex",
            &QPot::Static::currentIndex,
            "Get the index of the current minimum. Note:"
            "- 'index' : yielding position left"
            "- 'index + 1' : yielding position right")

        .def("__repr__", [](const QPot::Static&) { return "<QPot.Static>"; });
    ...
    py::class_<QPot::RedrawList>(m, "RedrawList")
        ...
        .def(
            "setProximity",
            &QPot::RedrawList::setProximity,
            "Customise proximity search",
            py::arg("proximity"))

        .def(
            "setPosition",
            &QPot::RedrawList::setPosition<xt::xtensor<double, 1>>,
        ...
        .def(
            "currentIndex",
            &QPot::RedrawList::currentIndex,
            "Get the index of the current minimum.")

        .def(
            "currentAnyRedraw",
            &QPot::RedrawList::currentAnyRedraw,
            "Check if there was a redraw the last time setPosition() was called.")

        .def(
            "currentRedraw",
            &QPot::RedrawList::currentRedraw,
            "Get the direction of redrawing the last time setPosition() was called.")

        .def(
            "currentRedrawRight",
            &QPot::RedrawList::currentRedrawRight,
            "List of particles that were redrawn to right, the last time setPosition() was called.")

        .def(
            "currentRedrawLeft",
            &QPot::RedrawList::currentRedrawLeft,
            "List of particles that were redrawn to left, the last time setPosition() was called.")

        .def(
            "redraw",
            &QPot::RedrawList::redraw<xt::xtensor<int, 1>>,
            "Force redraw (can be used to restore a sequence).",
            py::arg("iredraw"))

        .def(
            "redrawRight",
            &QPot::RedrawList::redrawRight<xt::xtensor<size_t, 1>>,
            "Force redraw (can be used to restore a sequence).",
            py::arg("index"))

        .def(
            "redrawLeft",
            &QPot::RedrawList::redrawLeft<xt::xtensor<size_t, 1>>,
            "Force redraw (can be used to restore a sequence).",
            py::arg("index"))

        .def("raw_val", &QPot::RedrawList::raw_val, "Current matrix of random values.")

        .def("raw_pos", &QPot::RedrawList::raw_pos, "Current matrix of yielding positions.")

        .def("raw_idx", &QPot::RedrawList::raw_idx, "Current matrix of indices.")

        .def("raw_idx_t", &QPot::RedrawList::raw_idx_t, "Current matrix of historic indices.")

        .def("__repr__", [](const QPot::RedrawList&) { return "<QPot.RedrawList>"; });
...

    {
        ...
        sm.def("seed", &SM::seed<size_t>, "Set seed.", py::arg("seed"));
